...
function quick_partition (start, end)
{
    ...
    div_update(divs[start],div_sizes[start],"#ffff80");//Color update
        ...
        for(var j =start + 1; j <= end ; j++ )
        {
            ...
            if (div_sizes[ j ] < piv)
            {
                ...
                div_update(divs[i],div_sizes[i],"#4db8ff");//Height update
                div_update(divs[j],div_sizes[j],"#4db8ff");//Height update

                i += 1;
            }
    }
    div_update(divs[start],div_sizes[start],"#ff704d");//Color update
